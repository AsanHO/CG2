...
vec2 Rasterization::ProjectWorldToRaster(vec3 point) {
    ...
    const float aspect = float(width) / height; // 16/9 = 1.7~~
    const vec2 pointNDC =
        vec2(point.x / aspect, point.y); // 정투영이기 때문에 z좌표 무시
...
void Rasterization::Render(vector<vec4> &pixels) {
    ...
    const auto xMin = std::min({v0.x, v1.x, v2.x});
    const auto yMin = std::min({v0.y, v1.y, v2.y});
    const auto xMax = std::max({v0.x, v1.x, v2.x});
    const auto yMax = std::max({v0.y, v1.y, v2.y});
    //cout << " 렌더링" << endl;
    ...
    for (size_t j = yMin; j <= yMax; j++) {
        for (size_t i = xMin; i <= xMax; i++) {
            ...
            const float alpha1 =
                EdgeFunction(v2, v0, point);
            const float alpha2 =
                EdgeFunction(v0, v1, point);
            //cout << alpha0 << " : " << alpha1 << " : " << alpha2 << endl;
            if (alpha0 > 0.0f && alpha1 > 0.0f && alpha2 > 0.0f) {
                //std::cout << "삼각형 감지" << endl;
                // 픽셀의 색 결정
                // 주의: 원근투영(perspective projection)에서는
                // depth 값을 고려해서 보정해줘야 합니다.
                 //cout << alpha0 << " : " << alpha1 << " : " << alpha2 << endl;
                // Bary-centric coordinates를 이용해서 color interpolation
                const auto r = alpha0 / (alpha0 + alpha1 + alpha2);
                const auto g = alpha1 / (alpha0 + alpha1 + alpha2);
                const auto b = alpha2 / (alpha0 + alpha1 + alpha2);
